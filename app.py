...
# Функция для предобработки изображения с холста
def preprocess_canvas_image(image_data):
    img_pil = Image.fromarray((image_data * 255).astype('uint8'))  # Создаем объект PIL.Image.Image
    img = img_pil.convert('L')  # Преобразование в оттенки серого
    resized_image = img.resize((28, 28), Image.BILINEAR)
    grayscaled_image = resized_image.convert("L")
    inverted_image = 255 - np.array(grayscaled_image)
    normalized_image = inverted_image / 255.0
    reshaped_image = normalized_image.reshape((1, 28, 28, 1))
    ...
    # Исходное изображение (преобразуем обратно в массив numpy для отображения в Matplotlib)
    ax[0, 0].set_title("Original Image")
    ax[0, 0].imshow(np.array(img_pil), cmap='gray')
