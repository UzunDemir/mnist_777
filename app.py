...
# Задаем параметры холста и загружаем фоновое изображение
drawing_mode = "freedraw"
stroke_width = 12
stroke_color = "black"  # Черный цвет для контура
bg_color = "white"  # Белый цвет фона
bg_image = st.sidebar.file_uploader("Фоновое изображение:", type=["png", "jpg"])
realtime_update = st.sidebar.checkbox("Обновление в реальном времени", True)
...
# Кнопка для предобработки и предсказания
if st.button('Классифицировать нарисованную цифру'):
    try:
        # Предобработка изображения с холста
        _, img_array = preprocess_image_from_path(canvas_result.image_data)
